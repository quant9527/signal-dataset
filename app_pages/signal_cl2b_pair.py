"""CL2B pair signal filter group page (url_path=signal_cl2b_pair).

页面提供一组预置的 signal 表筛选 SQL，分别展示匹配结果；
同时筛选出最近 5 天内出现过大涨的 symbol，再展示这些 symbol 的 cl2b_pair 信号。
"""
...
from data import get_surge_symbols, query_signals_by_rule
from signal_constants import AS_ALL_EXCHANGES, CL2B_PAIR_PREFIX
...
def _as_all_clause() -> str:
    """生成 AS 模式统一的 exchange 限制子句。"""
    return _AS_ALL_IN
...
# -----------------------------------------------------------------------------
_FILTER_SQLS: list[dict[str, str]] = [
    {
        "name": "cl2b_pair 全部",
        "description": "AS 模式范围内 signal_name 以 cl2b_pair 开头的全部信号",
        "where_clause": f"signal_name LIKE '{CL2B_PAIR_PREFIX}%' AND {_AS_ALL_IN}",
    },
    {
        "name": "cl2b_pair A 股个股",
        "description": "A 股个股（as）且 signal_name 以 cl2b_pair 开头",
        "where_clause": f"exchange = 'as' AND signal_name LIKE '{CL2B_PAIR_PREFIX}%'",
    },
    {
        "name": "cl2b_pair 大周期",
        "description": "AS 模式范围内 signal_name 以 cl2b_pair 开头且 freq 为大周期 (1d/1w/1M)",
        "where_clause": (
            f"signal_name LIKE '{CL2B_PAIR_PREFIX}%' "
            f"AND freq IN ('1d', '1w', '1M') AND {_AS_ALL_IN}"
        ),
    },
    {
        "name": "cl2b_pair 周线",
        "description": "AS 模式范围内 signal_name 以 cl2b_pair 开头且 freq 为 1w",
        "where_clause": f"signal_name LIKE '{CL2B_PAIR_PREFIX}%' AND freq = '1w' AND {_AS_ALL_IN}",
    },
    {
        "name": "cl2b_pair 做多方向",
        "description": "AS 模式范围内 signal_name 以 cl2b_pair 开头且 side 为 long",
        "where_clause": (
            f"signal_name LIKE '{CL2B_PAIR_PREFIX}%' AND side = 'long' AND {_AS_ALL_IN}"
        ),
    },
]


def _load_cl2b_pair_signals(days: int = 45) -> pd.DataFrame:
    """加载最近 N 天的 cl2b_pair 系列信号，并限制在 AS 模式范围内。"""
    df = get_cached_data(days, signal_name_prefix=CL2B_PAIR_PREFIX)
    if df.empty:
        return df
...
def _render_sql_filter_section(df_full: pd.DataFrame) -> None:
    ...
    if df_full.empty:
        st.info("暂无 cl2b_pair 信号数据，无法执行筛选 SQL。")
        return
...
def _render_surge_section(df_full: pd.DataFrame) -> None:
    """渲染「最近5天大涨 symbol 的 cl2b_pair 信号」区域。"""
    st.divider()
    st.subheader("2️⃣ 最近5天出现过大涨的 symbol 的 cl2b_pair 信号")
    st.caption(
        "仅 **exchange = as** 的标的；从 Flight (quant-lab) 拉取最近5天日线，"
        "计算日涨幅，筛选出任意一天涨幅 ≥ 7% 的 symbol，再展示其 cl2b_pair 信号。"
    )

    if df_full.empty:
        st.info("暂无 cl2b_pair 信号数据。")
        return
    ...
    # 仅 A 股个股参与 surge 判定（ths / asindex 不参与 Flight 日线拉取）
    as_df = df_full[df_full["exchange"] == "as"].copy()
    if as_df.empty:
        st.info("当前无 A 股（as）cl2b_pair 信号。")
        return
    ...
    days = st.number_input(
        ...
        value=5,
        step=1,
        key="cl2b_pair_surge_days",
    )

    if st.button("🔍 查询最近大涨 symbol", type="primary", key="cl2b_pair_surge_btn"):
        ...
        if not surge_symbols:
            st.info(
                f"最近 {int(days)} 天内，cl2b_pair 信号涉及的 A 股标的中 "
                f"无涨幅 ≥ {threshold}% 的记录。"
            )
            return
        ...
        if surge_df.empty:
            st.info("大涨标的中无 cl2b_pair 信号记录。")
            return

        st.success(
            f"最近 {int(days)} 天内共 **{len(surge_symbols)}** 个标的涨幅 ≥ {threshold}%，"
            f"其中涉及 **{surge_df['symbol'].nunique()}** 个标的的 cl2b_pair 信号。"
        )
        display_signals_multiview(surge_df, height=600, show_stats=True)


def page_signal_cl2b_pair() -> None:
    """CL2B pair 信号页面入口。"""
    st.set_page_config(layout="wide", page_title="CL2B Pair Signals")
    st.header("📊 CL2B Pair 信号筛选")
    st.markdown(f"""
**说明**：展示 `signal_name` 以 `{CL2B_PAIR_PREFIX}` 开头的信号。

- **第 1 区**：按预置 SQL 条件分别展示信号结果。
- **第 2 区**：筛选最近 5 天内出现过大涨的 A 股 symbol，展示其 cl2b_pair 信号。
""")
    st.divider()

    df_full = _load_cl2b_pair_signals(days=45)
    ...
    if not df_full.empty and "signal_date" in df_full.columns:
        ...
        st.info(f"📅 显示 {date_range[0]} 至 {date_range[1]} 的 cl2b_pair 信号。")
    else:
        st.warning("暂无 cl2b_pair 信号数据。")
